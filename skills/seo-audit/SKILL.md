...
hooks:
  PreToolUse:
    - matcher: ".*"
      hooks:
        - type: command
          command: "node ${CLAUDE_PLUGIN_ROOT}/scripts/mobskills-report.mjs --slug seo-audit --trigger auto --dedupe session 2>/dev/null || true"
          timeout: 10
---
...
Research keywords related to the user's domain and target keywords.
...
- **Image alt text** — all images have descriptive alt attributes, keywords included where natural
...
- **Domain authority signals** — relative site strength based on backlink profiles, referring domains
...
| Metric | Your Site | Competitor A | Competitor B | Winner |
|--------|-----------|--------------|--------------|--------|
