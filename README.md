...
This repo holds two things only: the plugin (skills + telemetry hooks) and the
marketplace manifest that installs it. The browsing, editing, and usage-analytics
UI lives in MobPulse under **Agent Skills**, which reads this repo through the
GitHub API — a skill merged here shows up there with no deploy and no sync job.
...
Then invoke a skill by its namespaced name — plugin skills are namespaced, so
`/verify-plan` on its own does not resolve:
...
plugin/skills/<slug>/              # optional: references/, scripts/, assets/,
                                   #   fonts/ — see moburst-deck-template
...
status: active                 # active | deprecated; deprecated gets a badge
...
2. Bump `version` on every edit. Reusing a version makes two revisions
   indistinguishable in analytics, retroactively — MobPulse rejects it.
3. **Keep the `## Usage reporting` section at the end of the body.** For
   ...
   `PostToolUse` hook cannot run inside Anthropic's sandbox. Frontmatter is
   protected on save (MobPulse merges rather than overwrites), but the body is
   replaced wholesale — drop that section and the skill goes silently dark.
